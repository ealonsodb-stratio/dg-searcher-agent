...
class SearchITTest extends FlatSpec {
  ...
  val httpManager: HttpManager = new DGHttpManager(AppConf.searcherManagerUrl, AppConf.searcherIndexerURL)
...
//  }
  ...
  "Partial indexation " should " index a document without errors" in {
    ...
    val res = httpManager.partialPostRequest(AppConf.searcherModel, data)
    val json = parse(res)
  ...
  "Total Indexation Error case " should " abort total indexation process" in {
    ...
    val init = httpManager.initTotalIndexationProcess(AppConf.searcherModel)
    ...
    httpManager.cancelTotalIndexationProcess(AppConf.searcherModel, token.get)
  ...
  "Total Indexation Succeed case " should " persist three registers" in {
    ...
    val res = httpManager.totalPostRequest(AppConf.searcherModel, token.get, data)
    val json = parse(res)
    val stats: Indexer  = json.extract[Indexer]
    ...
    // Finish process
    httpManager.finishTotalIndexationProcess(AppConf.searcherModel, token.get)
